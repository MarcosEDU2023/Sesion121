...
def process():
    ...
    if doc._.language["language"] == "en":
        ...
        elif task_option == "feelings":
            sid = SentimentIntensityAnalyzer()
            sentiment = sid.polarity_scores(raw_text)
            entities = [f"{key}: {value}" for key, value in sentiment.items()]
    ...
    if doc._.language["language"] == "es":
        ...
        if task_option == 'organization':
            ...
            entities=str(sentiment.sentiment("raw_text"))
                    
            return render_template('index.html', idioma=idioma, results=entities, num_of_results=1)
